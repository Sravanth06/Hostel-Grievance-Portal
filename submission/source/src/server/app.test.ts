...
describe('HostelGrievance API baseline', () => {
	...
	it('current-user works after login and fails after logout', async () => {
		...
		const after = await app.request('/api/me', {
			headers: { Cookie: cookie }
		});
	...
	it('student can create a grievance', async () => {
		...
		const res = await app.request('/api/grievances', {
			...
			body: JSON.stringify({
				...
				category: 'Room',
				description:
					'The cupboard hinge in B-204 is broken and the door will not close properly.'
			})
		});
		...
		expect(json.data.student.email).toBe(
			'student@example.test'
		);
	});
	...
	it('student can retrieve a permitted grievance', async () => {
		...
		expect(json.data.attachments[0].filename).toBe(
			'leaking-tap.jpg'
		);
	});
	...
	it('comments work for permitted users', async () => {
		...
		const res = await app.request(
			'/api/grievances/GRV-0001/comments',
			{
				...
				body: JSON.stringify({
					body: 'Following up on the leak this morning.'
				})
			}
		);
	...
	it('attachment metadata and storage work', async () => {
		...
		const created = await app.request(
			'/api/grievances',
			{
				...
				body: JSON.stringify({
					...
					category: 'Other',
					description:
						'Filing this so I can attach a photo of the damaged locker door.'
				})
			}
		);
		...
		expect(fileRes.headers.get('content-type')).toBe(
			'image/png'
		);
	...
	it('rejects oversized and disallowed attachments', async () => {
		...
		const overRes = await app.request(
			'/api/grievances/GRV-0008/attachments',
			{
				method: 'POST',
				headers: { Cookie: cookie },
				body: over
	...
	it('lets a student edit their own open grievance but not a resolved one', async () => {
		...
		const edited = await app.request(
			'/api/grievances/GRV-0008',
			{
				...
				body: JSON.stringify({
					title: 'Mess tables still dirty before dinner'
				})
			}
		);
		...
		expect(editedJson.data.title).toContain(
			'still dirty'
		);
		...
		const forbidden = await app.request(
			'/api/grievances/GRV-0008',
			{
				...
				body: JSON.stringify({
					title: 'Should not work at all here'
				})
			}
		);
		...
		const resolved = await app.request(
			'/api/grievances/GRV-0004',
			{
				...
				body: JSON.stringify({
					title: 'Trying to change a resolved ticket'
				})
			}
		);
	...
	it('returns 404 for unknown grievance ids without leaking internals', async () => {
		...
		expect(
			JSON.stringify(json)
		).not.toMatch(/sqlite|stack|ENOENT/i);
	});
});