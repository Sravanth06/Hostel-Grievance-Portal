...
export function seedDatabase(
	 ...
	 * password: warden123
	 *
	 * These are already generated Argon2id hashes so the
	 * seed operation remains synchronous. This is important
	 * because the existing test suite calls seedDatabase()
	 * without await.
	 */
	const studentHash =
		'$argon2id$v=19$m=65536,t=3,p=4$T92IcHO7b8tGCvtyz/fbuw$P8dtnYT7hkRsnOottEARTVA1bTDGge3zApTILYmAfRk';

	const wardenHash =
		'$argon2id$v=19$m=65536,t=3,p=4$j1/0mfBwtMiCYs/oH73+5Q$2gBxQPbNeJlmoRqe9M7zyHc4KT4jrHelGzeEKNLX7vM';
	...
	const grievances = [
		...
		},
		{
			...
			category: 'Internet',
			description:
				'The Wi-Fi in Block A disconnects repeatedly, especially between 8pm and midnight. Speedtests show under 1 Mbps when connected. Attached a screenshot from yesterday.',
			status: 'open',
	...
	/*
	 * Tiny valid-ish image buffers used only for seeded
	...
	db.transaction(() => {
		...
		for (const file of files) {
			const stored = newStoredName(
				file.mime_type
			);

			writeFileSync(
				join(uploadsDir, stored),
				file.bytes
			);
...
/*
 * Generate an unpredictable filename for stored
 * attachment files.
 */
function newStoredName(
	mimeType: string
): string {
	const extension =
		mimeType === 'image/jpeg'
